package osu.sladcik.agents.AdaptiveAgents.GUI;
...
import osu.sladcik.agents.AdaptiveAgents.GuiAgent;
...
public class MainFrame extends JFrame {
    ...
    public static String statusRadiator;
    ...
    private void showRadiatorStatus() {
        Thread tempThread = new Thread() {
            ...
            public void run() {
                while (true){
                    radiatorLabel.setText(statusRadiator);
                    try {
                        Thread.sleep(100);
    ...
    public void showWindowStatus(){
        Thread tempThread = new Thread() {
            ...
            public void run() {
                while (true){
                    windowLabel.setText(statusWindow);
                    try {
                        Thread.sleep(100);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
    ...
    public static void main(String[] args) {
        ...
        try {
            ac = cc.createNewAgent(TypeAgents.GUI.toString(), "osu.sladcik.agents.AdaptiveAgents.GuiAgent", null);
            ac.start();
