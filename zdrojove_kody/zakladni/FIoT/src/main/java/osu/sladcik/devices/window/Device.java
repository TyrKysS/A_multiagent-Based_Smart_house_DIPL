package osu.sladcik.devices.window;
...
public class Device extends Agent {
    ...
    private boolean setWindow = false;
            ...
            public void action() {
                ...
                if (!message.isEmpty() && firstMessage) {
                    ...
                    if (splitMessage[1].equals(targetAgent[0])){
                        setWindow = Boolean.parseBoolean(splitMessage[0]);
                        Messages.sendMessageStatus(getLocalName(), setWindow);
                    }
                }
            }
        });
