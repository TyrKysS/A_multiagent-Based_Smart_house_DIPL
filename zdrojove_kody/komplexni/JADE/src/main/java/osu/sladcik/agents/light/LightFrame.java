...
public class LightFrame extends JFrame {
    ...
    LightFrame(){
        ...
        setLightValue.addActionListener(e -> {
            actualLightValue = Integer.parseInt(lightValue.getText());
        });
            ...
            while(true){
                ...
                for (int[] ints : matrix) {
                    for (int j = 0; j < ints.length; j++) {
                        if (ints[0] == lightStatus && ints[1] == counter && ints[2] <= actualLightValue && ints[3] > actualLightValue && ints[4] == isFire) {
                            if (ints[5] == 5){
                                int max = 4;
                                int min = 1;
                                int range = max - min + 1;
